...
class WP_Sync_AmoCRM_WPCF7
{
    public function __construct($form_id)
    {
        ...
        try {
            ...
            $lead_name = isset($_POST[$forms_option[$form_id]['name']]) ? esc_attr($_POST[$forms_option[$form_id]['name']]) : '';
            ...
            $lead_phone = isset($_POST[$forms_option[$form_id]['phone']]) ? esc_attr($_POST[$forms_option[$form_id]['phone']]) : null;
            $phone->setValues(
            ...
            $lead_email = isset($_POST[$forms_option[$form_id]['email']]) ? esc_attr($_POST[$forms_option[$form_id]['email']]) : null;
            $email->setValues(
                (new MultitextCustomFieldValueCollection())
                    ->add(
                        (new MultitextCustomFieldValueModel())
            ...
            $lead = new LeadModel();
            $lead->setPipelineId(esc_attr($forms_option[$form_id]['pipeline']));
            $lead->setStatusId(esc_attr($forms_option[$form_id]['status']));
            $lead->setResponsibleUserId(esc_attr($forms_option[$form_id]['user_responsible']));
            $lead->setName($lead_name);

            $tags = new TagsCollection();
            foreach (explode(',', esc_attr($forms_option[$form_id]['tags'])) as $name) {
                $tag = new TagModel();
                $tag->setName($name);
                $tags->add($tag);
            }
